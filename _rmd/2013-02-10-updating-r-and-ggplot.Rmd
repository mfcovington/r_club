...
# Check your current version of R and ggplot
...
The current version of R is `2.15.2` and the current version of ggplot is `ggplot2_0.9.3` (found under 'other attached packages').

If you have the most recent version of R, but your ggplot is somehow out of date, run: {% highlight r %}update.packages("ggplot2"){% endhighlight %}. (Or if you don't have ggplot installed, run {% highlight r %}install.packages("ggplot2"){% endhighlight %})

If both R and ggplot are old, first install R and then update your packages.
